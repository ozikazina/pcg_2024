...
int main(int argc, char **argv)
{
  ...
  cudaStream_t computeStream, writeStream, comStream;
  cudaStreamCreateWithFlags(&computeStream, cudaStreamNonBlocking);
  cudaStreamCreateWithFlags(&writeStream, cudaStreamNonBlocking);
  cudaStreamCreateWithFlags(&comStream, cudaStreamNonBlocking);

  cudaEvent_t canUseSrcEvent, memcpyDoneEvent, comDoneEvent;
  cudaEventCreate(&canUseSrcEvent, cudaEventDisableTiming);
  cudaEventCreate(&memcpyDoneEvent, cudaEventDisableTiming);
  cudaEventCreate(&comDoneEvent, cudaEventDisableTiming);
  ...
  for (unsigned s = 0u; s < steps; ++s)
  {
    ...
    if (shouldWrite(s)) {
      // Center of mass
      cudaMemsetAsync(dCenterOfMass, 0, 4 * sizeof(float), comStream);
      centerOfMass<<<redGridDim, redBlockDim, redSharedMemSize, comStream>>>(dParticles[srcIdx], dCenterOfMass, dLock, N);
      cudaEventRecord(comDoneEvent, comStream);

      // Next iter
      calculateVelocity<<<simGridDim, simBlockDim, sharedMemSize, computeStream>>>(dParticles[srcIdx], dParticles[dstIdx], N, dt);
      cudaEventRecord(canUseSrcEvent, computeStream);

      // Writes
      CUDA_CALL(cudaMemcpyAsync(hParticles.posX, dParticles[srcIdx].posX, N * sizeof(float), cudaMemcpyDeviceToHost, writeStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.posY, dParticles[srcIdx].posY, N * sizeof(float), cudaMemcpyDeviceToHost, writeStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.posZ, dParticles[srcIdx].posZ, N * sizeof(float), cudaMemcpyDeviceToHost, writeStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.velX, dParticles[srcIdx].velX, N * sizeof(float), cudaMemcpyDeviceToHost, writeStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.velY, dParticles[srcIdx].velY, N * sizeof(float), cudaMemcpyDeviceToHost, writeStream));
      CUDA_CALL(cudaMemcpyAsync(hParticles.velZ, dParticles[srcIdx].velZ, N * sizeof(float), cudaMemcpyDeviceToHost, writeStream));
      cudaStreamWaitEvent(writeStream, comDoneEvent);
      CUDA_CALL(cudaMemcpyAsync(hCenterOfMass, dCenterOfMass, 4 * sizeof(float), cudaMemcpyDeviceToHost, writeStream));
      cudaEventRecord(memcpyDoneEvent, writeStream);
      
      cudaStreamWaitEvent(comStream, canUseSrcEvent);
      cudaStreamWaitEvent(writeStream, canUseSrcEvent);

      // Host write
      cudaEventSynchronize(memcpyDoneEvent);
  ...
  cudaStreamDestroy(comStream);

  cudaEventDestroy(canUseSrcEvent);
  cudaEventDestroy(memcpyDoneEvent);
  cudaEventDestroy(comDoneEvent);
