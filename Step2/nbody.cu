...
__global__ void calculateVelocity(Particles pIn, Particles pOut, const unsigned N, float dt)
{
  ...
  for (int k = 0; k < (N - 1) / SH_SIZE + 1; ++k)
  {
    ...
    if (shouldLoad) {
      mem[threadIdx.x + 0 * SH_SIZE] = pIn.posX[sIdx];
      mem[threadIdx.x + 1 * SH_SIZE] = pIn.posY[sIdx];
      mem[threadIdx.x + 2 * SH_SIZE] = pIn.posZ[sIdx];
      mem[threadIdx.x + 3 * SH_SIZE] = pIn.velX[sIdx];
      mem[threadIdx.x + 4 * SH_SIZE] = pIn.velY[sIdx];
      mem[threadIdx.x + 5 * SH_SIZE] = pIn.velZ[sIdx];
      mem[threadIdx.x + 6 * SH_SIZE] = pIn.weight[sIdx];
    }
    ...
    {
      const float otherPosX = mem[j + 0 * SH_SIZE];
      const float otherPosY = mem[j + 1 * SH_SIZE];
      const float otherPosZ = mem[j + 2 * SH_SIZE];
      const float otherWeight = mem[j + 6 * SH_SIZE];
      ...
      if (r > COLLISION_DISTANCE) {
        ...
        const float otherVelX = mem[j + 3 * SH_SIZE];
        const float otherVelY = mem[j + 4 * SH_SIZE];
        const float otherVelZ = mem[j + 5 * SH_SIZE];
